...
import { Plus, Edit, Trash2, Download, RefreshCw, LogOut, User, Database } from 'lucide-react';
...
const AdminPage = () => {
  ...
  const { games, saveGames, blogPosts, saveBlogPosts, resetToOriginalData, exportData } = useData();
  ...
  const handleResetData = () => {
    if (window.confirm('¿Estás seguro de que quieres resetear todos los datos a los valores originales? Esta acción no se puede deshacer.')) {
      resetToOriginalData();
      toast({
        title: "Datos reseteados",
        description: "Todos los datos han sido restaurados a los valores originales"
      });
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <Card className="glass-effect border-white/10">
              ...
              <CardContent>
                ...
                <p className="text-gray-400 text-sm mt-3">
                  Exporta un backup de todos los datos o resetea a los valores originales del código fuente.
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <Tabs defaultValue="games" className="w-full">
            <TabsList className="grid w-full grid-cols-2 glass-effect mb-8">
              <TabsTrigger value="games">Gestionar Juegos ({games.length})</TabsTrigger>
              <TabsTrigger value="blog">Gestionar Blog ({blogPosts.length})</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="games">
              ...
              >
                ...
                <Card className="glass-effect border-white/10">
                  <CardHeader>
                    <CardTitle className="text-white">Juegos en el Catálogo</CardTitle>
                  </CardHeader>
                  <CardContent>
                      ...
                      {games.map((game) => (
                        <div key={game.id} className="flex items-center justify-between p-4 glass-effect rounded-lg border border-white/10">
                          <div className="flex-1">
                            ...
                            <p className="text-gray-400 text-sm">{game.categories.join(', ')} • {game.players} jugadores</p>
                            {game.rating && (
                          ...
                          <div className="flex space-x-2">
                            <Button onClick={() => handleEditGame(game)} variant="outline" size="sm" className="border-blue-500/30 text-blue-400 hover:bg-blue-500/10">
                              <Edit className="h-4 w-4" />
                            </Button>
                            <Button onClick={() => handleDeleteGame(game.id)} variant="outline" size="sm" className="border-red-500/30 text-red-400 hover:bg-red-500/10">
            ...
            <TabsContent value="blog">
              ...
              >
                ...
                <Card className="glass-effect border-white/10">
                  <CardHeader>
                    <CardTitle className="text-white">Artículos del Blog</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4 max-h-96 overflow-y-auto">
                      {blogPosts.map((post) => (
                        <div key={post.id} className="flex items-center justify-between p-4 glass-effect rounded-lg border border-white/10">
                          <div className="flex-1">
                            ...
                            <p className="text-gray-400 text-sm">{post.category} • {post.date} • {post.readTime}</p>
                            {post.featured && (
                              <Badge className="bg-yellow-500 text-black text-xs mt-1">Destacado</Badge>
                            )}
                          </div>
                          <div className="flex space-x-2">
                            <Button onClick={() => handleEditPost(post)} variant="outline" size="sm" className="border-blue-500/30 text-blue-400 hover:bg-blue-500/10">
                              <Edit className="h-4 w-4" />
                            </Button>
                            <Button onClick={() => handleDeletePost(post.id)} variant="outline" size="sm" className="border-red-500/30 text-red-400 hover:bg-red-500/10">
                              <Trash2 className="h-4 w-4" />
                            </Button>
                          </div>
                        </div>
